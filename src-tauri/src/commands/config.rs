use crate::config::{sanitize_active_sprite_set, AppConfig};
...
pub fn get_config(app: AppHandle) -> Result<AppConfig, String> {
    ...
    let config = AppConfig {
        work_interval_minutes: store
            .get("work_interval_minutes")
            .and_then(|v| v.as_u64())
            .unwrap_or(45),
        rest_duration_minutes: store
        ...
        active_sprite_set: sanitize_active_sprite_set(
            store
                .get("active_sprite_set")
                .and_then(|v| v.as_str().map(String::from))
...
pub fn save_config(app: AppHandle, config: AppConfig) -> Result<(), String> {
    ...
    let active_sprite_set = sanitize_active_sprite_set(config.active_sprite_set);
    store.set("active_sprite_set", &*active_sprite_set);
