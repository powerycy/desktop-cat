use std::sync::{Arc, Mutex};
use std::time::Duration;
use tauri::{AppHandle, Emitter};
use tauri::async_runtime;
use tokio::time::interval;

pub struct TimerState {
    pub elapsed_seconds: u64,
    pub work_interval_seconds: u64,
    pub rest_duration_seconds: u64,
    pub is_resting: bool,
    pub paused: bool,
}

impl TimerState {
    pub fn new(work_minutes: u64, rest_minutes: u64) -> Self {
        Self {
            elapsed_seconds: 0,
            work_interval_seconds: work_minutes * 60,
            rest_duration_seconds: rest_minutes * 60,
            is_resting: false,
            paused: false,
        }
    }
}

/// Starts the background timer loop.
/// Emits:
///   - "timer-tick"    { elapsed, total, is_resting, rest_remaining }  every second
///   - "rest-start"    { mode, duration }  when work interval is reached
///   - "rest-end"      {}                  when rest period is over
pub fn start_timer(app: AppHandle, state: Arc<Mutex<TimerState>>, rest_mode: String) {
    async_runtime::spawn(async move {
        let mut ticker = interval(Duration::from_secs(1));
        ticker.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);

        loop {
            ticker.tick().await;

            let (should_start_rest, should_end_rest, elapsed, total, is_resting, rest_remaining) = {
                let mut s = state.lock().unwrap();
                if s.paused {
                    continue;
                }

                s.elapsed_seconds += 1;

                let mut should_start_rest = false;
                let mut should_end_rest = false;

                if !s.is_resting && s.elapsed_seconds >= s.work_interval_seconds {
                    s.is_resting = true;
                    s.elapsed_seconds = 0;
                    should_start_rest = true;
                } else if s.is_resting && s.elapsed_seconds >= s.rest_duration_seconds {
                    s.is_resting = false;
                    s.elapsed_seconds = 0;
                    should_end_rest = true;
                }

                let rest_remaining = if s.is_resting {
                    s.rest_duration_seconds.saturating_sub(s.elapsed_seconds)
                } else {
                    0
                };
                let total = if s.is_resting {
                    s.rest_duration_seconds
                } else {
                    s.work_interval_seconds
                };

                (should_start_rest, should_end_rest, s.elapsed_seconds, total, s.is_resting, rest_remaining)
            };

            // Emit timer-tick every second
            let _ = app.emit("timer-tick", serde_json::json!({
                "elapsed": elapsed,
                "total": total,
                "is_resting": is_resting,
                "rest_remaining": rest_remaining,
            }));

            if should_start_rest {
                let duration = {
                    let s = state.lock().unwrap();
                    s.rest_duration_seconds
                };
                let _ = app.emit("rest-start", serde_json::json!({
                    "mode": rest_mode,
                    "duration": duration,
                }));
            }

            if should_end_rest {
                let _ = app.emit("rest-end", serde_json::json!({}));
            }
        }
    });
}
